...
export async function testProviderConnection() {
  ...
  try {
    ...
    if (category === "authentication_401" || category === "permission_403") {
      ...
      // Record the condition for diagnostics, but do not permanently block the
      // provider. A later request may succeed after a temporary 429 clears.
      providerQuotaExhausted = true;
      quotaExhaustedProvider = providerLabel();
      quotaExhaustedAt       = new Date().toISOString();
      logger.warn(`[AI Core] [QUOTA_EXHAUSTED] ${safe.providerReason} — diagnostic state recorded for ${quotaExhaustedProvider}`);
    } else {
...
export async function validateProviderModel(model) {
  ...
  // A previous 429 must not permanently block validation. Quota/rate-limit
  // state is diagnostic only; the provider is the source of truth.
  const apiKey = getActiveApiKey();
...
async function requestModel(messages, maxTokens = config().maxResponse) {
  // Do not permanently block the provider after a previous 429.
  // A 429 can be temporary, model-specific, project-specific, or quota-related.
  // The provider response remains the source of truth for each new request.
  const apiKey = getActiveApiKey();
  ...
  try {
    ...
    // Keep quota state for diagnostics only. Do not permanently block future
    // requests: a 429 may be temporary or scoped to a specific model/project.
    if (safe.providerCategory === "quota_exhausted") {
      ...
      logger.warn(
        ...
        `— diagnostic state recorded; future requests are not permanently blocked.`
      );
    }
